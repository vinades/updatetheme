...
if ($nv_Request->isset_request('save', 'post')) {
    ...
    foreach ($files as $file) {
        ...
        if (preg_match('/' . nv_preg_quote($theme_update) . '\/css\/style\.css$/', $file)) {
            ...
            if ($n[1] == 'users') {
                // users theme.php
                require NV_ROOTDIR . '/modules/' . $module_file . '/' . $op . '/users-theme.php.php';
            }
        } elseif (preg_match('/' . nv_preg_quote($theme_update) . '\/js\/users\.js$/i', $file, $n)) {
            // users.js
            require NV_ROOTDIR . '/modules/' . $module_file . '/' . $op . '/users.js.php';
