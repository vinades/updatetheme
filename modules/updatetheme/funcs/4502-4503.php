...
if ($nv_Request->isset_request('save', 'post')) {
    ...
    foreach ($files as $file) {
        ...
        if (preg_match('/' . nv_preg_quote($theme_update) . '\/modules\/(.*?)\/(.*?)\.tpl$/', $file, $n)) {
            if ($n[1] == 'news') {
        ...
        } elseif (preg_match('/' . nv_preg_quote($theme_update) . '\/css\/news\.css$/i', $file, $n)) {
            // news.css
            require NV_ROOTDIR . '/modules/' . $module_file . '/' . $op . '/news.css.php';
        }
